...
- **The uniqueness + premium mandate (operator directive, 2026-07-11).** Every demo AND every final client site: (1) is grounded in the ui-ux-pro-max skill (the uiux agent queries it per lead and records `skill_grounded` on the design; a design that skips grounding is a bug), (2) draws its structural range from the looks registry, whose looks are skill-synthesized and whose hero structures are sourced from 21st.dev patterns restyled through §5b-bis (the 21st MCP only exists in interactive sessions, so it feeds the registry and template variants at build time, not per-lead at runtime — that is the honest mechanic), and (3) must read as designed-for-this-business: distinct palette, distinct type pairing, distinct hero structure from every other active demo in the metro. Five hero structures exist (photo, split, bold, frame, paper — "paper" opens light/editorial, the rest dark/atmospheric); 34 looks across roofing (14), plumbing (10), HVAC (5), dental (5). When the free-look pool for a niche+metro runs thin, expanding the registry (new skill-grounded looks, new 21st-sourced structures) comes BEFORE reusing anything.
