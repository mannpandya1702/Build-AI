...
- **Installed design toolchain (2026-07-03):** the **ui-ux-pro-max** skill (`.claude/skills/ui-ux-pro-max`, 161 color palettes, 85 styles, 1924 fonts, 99 UX guidelines, 162 product types, queryable via `python3 .claude/skills/ui-ux-pro-max/scripts/search.py "<query>" --domain color|style|typography|ux|product|chart`) and the **21st.dev MCP** (component search/generation, live from the next session). Use the skill to ground palette/type/style choices in its database instead of guessing, then apply the anti-slop rules in 5b-bis to what it returns. The skill informs; the contract still governs (no purple gradients, accent-for-CTAs-only, section rhythm, real photos).
