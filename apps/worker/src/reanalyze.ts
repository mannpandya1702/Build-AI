...
for (const name of names) {
  ...
  try {
    ...
    const after = (await pool.query<{ status: string }>("select status from leads where id=$1", [id])).rows[0]
      .status;
    if (after === "analyzed") await realSolution(id);
    const audit = (
      await pool.query(
        "select lighthouse, jsonb_array_length(coalesce(findings,'[]')) n, summary, jsonb_array_length(coalesce(screenshots,'[]')) shots from audits where lead_id=$1 order by created_at desc limit 1",
        [id],
      )
    ).rows[0];
    console.log(
      `  audit: ${audit?.n ?? 0} findings | ${audit?.shots ?? 0} screenshots | lighthouse=${audit?.lighthouse ? "yes" : "none"}`,
    );
    console.log(`  summary: ${audit?.summary ?? "(none)"}`);
    const finalStatus = (await pool.query<{ status: string }>("select status from leads where id=$1", [id]))
      .rows[0].status;
