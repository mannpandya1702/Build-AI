...
export default function SettingsPage() {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <PageHeader title="Settings" description="Outreach mode and ICP overrides. Caps live in config/caps.yaml." />

      <Card className="p-4">
        <SectionTitle>Outreach mode</SectionTitle>
        <label htmlFor="mode" className="mt-3 block text-sm text-muted">How outbound email leaves the system</label>
        ...
        <p className="mt-1.5 text-xs text-faint">Review is the safe default. Auto respects daily caps and the suppression list either way.</p>
      </Card>

      <Card className="mt-4 p-4">
        <SectionTitle>ICP overrides</SectionTitle>
        <label htmlFor="icp" className="mt-3 block text-sm text-muted">Merged over config/icp.yaml at runtime (JSON)</label>
        <textarea id="icp" value={icp} onChange={(e) => setIcp(e.target.value)} rows={12} spellCheck={false}
      ...
      <div className="mt-4 flex items-center gap-3">
        <button onClick={save} className="h-10 cursor-pointer rounded-lg bg-accent px-5 font-display text-sm font-semibold text-accentink transition-opacity duration-150 hover:opacity-90">
          Save
        </button>
        {saved && <span className={`text-sm ${saved.startsWith("error") ? "text-danger" : "text-ok"}`} role="status">{saved}</span>}
      </div>
    </div>
  );
}
