                  ...
                  {expanded && (
                    <div className="border-t border-line px-3 py-3">
                      ...
                      <div className="mt-3 flex gap-2">
                        <button
                          disabled={busy}
                          onClick={() => act(() => post("/api/outbox/approve", { emailId: e.id }))}
                          className="h-9 cursor-pointer rounded-lg bg-accent px-4 font-display text-sm font-semibold text-accentink transition-opacity duration-150 hover:opacity-90 disabled:opacity-50"
                        >
                          Approve & send
                        </button>
                        <button
                          disabled={busy}
                          onClick={() => act(() => post("/api/outbox/reject", { emailId: e.id }))}
                          className="h-9 cursor-pointer rounded-lg border border-line px-4 text-sm text-muted transition-colors duration-150 hover:bg-surface2 hover:text-ink disabled:opacity-50"
                        >
                          Reject
                        </button>
                        <Link
              ...
              o.callsDue.map((c) => (
                <Card key={c.lead_id} className="flex flex-wrap items-center gap-2 px-3 py-2 text-sm">
                  ...
                  {devTools && (
                    <button
                      disabled={busy}
                      onClick={() => act(() => post("/api/dev/simulate-booking", { leadId: c.lead_id }))}
                      className="cursor-pointer rounded-md border border-line px-2 py-1 font-display text-[11px] text-muted transition-colors duration-150 hover:bg-surface2"
                    >
                      simulate booking
                    </button>
                  )}
