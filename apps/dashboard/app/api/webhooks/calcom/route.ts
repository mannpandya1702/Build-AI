...
export async function POST(req: Request) {
  ...
  const lead = email
    ? await db().query<{ id: string }>("select id from leads where lower(contact_email)=lower($1) limit 1", [
        email,
      ])
  ...
  if (!lead.rowCount) {
    ...
    await db().query("insert into notifications (type, title, body) values ('booking_unmatched', $1, $2)", [
      `Cal.com booking from ${email ?? "unknown"} matched no lead`,
      raw.slice(0, 500),
    ]);
