...
interface BatchState { size: number; used: number; remaining: number; started_at: string | null }
interface WorkerState { enabled: boolean; online: boolean; last_heartbeat: string | null; demo_batch: BatchState | null }
...
export default function WorkerSwitch() {
  ...
  async function toggle() {
    ...
    if (next && !confirm(`Turn the worker ON?\n\nIt will start processing everything queued: discovery requests, demo builds (real Vercel deploys + AI spend, 2 at a time, best leads first), and approved sends. ${batchLine} Caps and budgets apply.`)) return;
    setBusy(true);
  ...
  async function setBatch(size: number | null) {
    if (batchBusy) return;
    setBatchBusy(true);
    await fetch("/api/worker", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ batch_size: size }) });
    setN("");
    await refresh();
    setBatchBusy(false);
  }
      ...
      <div className="mt-2 border-t border-line/60 pt-2">
        {b ? (
          <div className="mb-1.5">
            ...
            <div className="mt-1 h-1 overflow-hidden rounded-full bg-surface2" role="progressbar" aria-valuenow={b.used} aria-valuemin={0} aria-valuemax={b.size}>
              <div className="h-full rounded-full bg-data transition-[width] duration-300" style={{ width: `${pct}%` }} />
            </div>
            <p className="mt-1 text-[10px] leading-tight text-faint">
              {b.remaining > 0 ? `${b.remaining} left, best scores first` : "batch done, set a new number for more"}
            </p>
          </div>
        ) : (
          <p className="mb-1.5 text-[10px] leading-tight text-faint">No demo limit: every qualified lead gets a demo.</p>
        )}
