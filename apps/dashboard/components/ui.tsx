...
export function statusTone(status: string): { text: string; bg: string; ring: string; dot: string } {
  ...
  if (LOST.has(status)) return { text: "text-danger", bg: "bg-danger/10", ring: "ring-danger/25", dot: "bg-danger" };
  if (BLOCKED.has(status)) return { text: "text-warn", bg: "bg-warn/10", ring: "ring-warn/25", dot: "bg-warn" };
...
export function StatusPill({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 whitespace-nowrap rounded-full px-2 py-0.5 font-display text-[11px] font-medium ring-1 ${t.bg} ${t.text} ${t.ring}`}>
...
export function SectionTitle({ children }: { children: React.ReactNode }) {
  return <h2 className="font-display text-[11px] font-semibold uppercase tracking-[0.14em] text-faint">{children}</h2>;
}
...
export function StatTile({ label, value, sub, tone = "ink", href }: { label: string; value: React.ReactNode; sub?: string; tone?: "ok" | "data" | "warn" | "danger" | "ink"; href?: string }) {
  const toneText = { ok: "text-ok", data: "text-data", warn: "text-warn", danger: "text-danger", ink: "text-ink" }[tone];
  const inner = (
    <div className="rounded-card border border-line bg-surface p-4 transition-colors duration-150 hover:border-faint/40">
      ...
      <p className={`mt-1.5 font-display text-[26px] font-semibold leading-none tracking-tight ${toneText}`}>{value}</p>
...
export function ScoreDial({ label, score }: { label: string; score: number | null }) {
  ...
  const ring = score == null ? "rgb(var(--border))" : v >= 90 ? "rgb(var(--ok))" : v >= 50 ? "rgb(var(--warn))" : "rgb(var(--danger))";
  return (
    <div className="flex flex-col items-center gap-1.5">
      <div className="relative h-16 w-16">
        <svg viewBox="0 0 36 36" className="h-16 w-16 -rotate-90">
          <circle cx="18" cy="18" r="15.5" fill="none" stroke="rgb(var(--surface-2))" strokeWidth="3" />
          <circle cx="18" cy="18" r="15.5" fill="none" stroke={ring} strokeWidth="3" strokeLinecap="round" strokeDasharray={`${(v / 100) * 97.4} 97.4`} />
        </svg>
        <span className={`absolute inset-0 flex items-center justify-center font-display text-sm font-semibold ${color}`}>
          {score == null ? "—" : v}
        </span>
      </div>
...
export function Empty({ icon = "inboxEmpty", children, hint }: { icon?: IconName; children: React.ReactNode; hint?: string }) {
  return (
