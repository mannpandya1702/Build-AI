...
import { MOCK, isUnconfirmed, loadAgencyFacts, loadCaps } from "./config.js";
...
export async function emailGate({ toEmail, subject, bodyText }: GateInput): Promise<GateResult> {
  ...
  if (!subject.trim() || /re:|fwd:/i.test(subject.slice(0, 4)))
    return { ok: false, reason: "subject not honest/present" };
  if (!bodyText.includes("unsubscribe")) return { ok: false, reason: "missing unsubscribe line" };
  const footer = canSpamFooter();
  if (!footer || !bodyText.includes(loadAgencyFacts().identity.address))
    return { ok: false, reason: "missing physical address footer" };
  const caps = loadCaps();
