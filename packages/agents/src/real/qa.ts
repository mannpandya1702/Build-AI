...
const VISION_PROMPT = readFileSync(
  resolve(dirname(fileURLToPath(import.meta.url)), "prompts/qa-vision.md"),
  "utf8",
);
...
export async function qa(leadId: string): Promise<void> {
  ...
  const build = (
    await pool.query("select * from builds where lead_id=$1 and kind=$2 order by created_at desc limit 1", [
      leadId,
      kind,
    ])
  ).rows[0];
  ...
  const checks: Check[] = [];
  const push = (name: string, ok: boolean, detail: string, critical = true) =>
  ...
  if (content) {
    if (kind === "demo") {
      ...
      push(
        "watermark on demo",
        Boolean(content.demo) && Boolean(content.watermark),
        content.watermark ? "present" : "missing",
      );
    } else {
      push(
        "no watermark on final",
        !content.demo && !content.watermark,
        content.watermark ? "watermark still present" : "clean",
      );
    ...
    push(
      ...
      content.phone ? `phone ${content.phone}` : "no phone on GBP (form-only, flagged)",
    ...
    push(
      ...
      !badReview,
      badReview
        ? `a ${badReview.rating}-star review is shown`
        : `${(content.reviews ?? []).length} curated reviews`,
    );
  ...
  for (const f of sourceFiles) {
    ...
    if (m) {
      secretHit = `${f.split("/").pop()} matches ${m}`;
      break;
    }
  }
  ...
  if (isLive) {
    ...
    push(
      "publicly reachable",
      Boolean(reach && reach.status === 200),
      `status ${reach?.status ?? "unreachable"}`,
    );
    ...
    try {
      ...
      push(
        "mobile performance",
        psi.performance >= perfBar,
        `Lighthouse perf ${psi.performance} (bar ${perfBar})`,
        kind === "final" ? true : psi.performance < 50,
      );
      ...
      if (kind === "final") push("seo", psi.seo >= 90, `Lighthouse SEO ${psi.seo} (bar 90)`, true);
        ...
        push(
          "seo (demo, noindex-capped)",
          true,
          `Lighthouse SEO ${psi.seo}; expected low because the demo is noindexed`,
          false,
        );
    } catch (err) {
    ...
    try {
      ...
      const raw = await llm({
        ...
        images: imgs,
        prompt: "Review the attached deployed-site screenshots and return the JSON verdict.",
        mockResponse: JSON.stringify({ pass: true, issues: [] }),
      });
      const v = safeJson(raw);
      const visionIssues = Array.isArray(v?.issues) ? v.issues.filter((i: any) => i.severity !== "low") : [];
      push(
        "visual review",
        v?.pass !== false && visionIssues.length === 0,
        visionIssues.length ? visionIssues.map((i: any) => `${i.where}: ${i.what}`).join("; ") : "clean",
        false,
      );
    } catch (err) {
  ...
  if (passed) {
    await emitEvent({
      ...
      leadId,
      type: "qa.passed",
      message: `${kind} #${iteration} green (${checks.length} checks)${MOCK() ? " (mock)" : ""}`,
    });
    if (kind === "demo") {
      ...
      await notifyOperator({
        ...
        title: `${lead.company_name}: final build ready for delivery approval`,
        leadId,
      });
    }
    return;
  ...
  await emitEvent({
    ...
    level: "warn",
    type: "qa.failed",
    message: `${kind} #${iteration} failed: ${criticalFailing.map((c) => c.name).join(", ")}`,
    payload: { issues },
  });
  if (iteration >= 2) {
    await notifyOperator({
      type: "qa_failed",
      title: `${lead.company_name}: ${kind} failed QA twice, needs you`,
      leadId,
    });
    await emitEvent({
      agent: "qa",
      ...
      level: "warn",
      type: "qa.held",
      message: `held after ${iteration} iterations`,
    });
