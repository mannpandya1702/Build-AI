...
async function getLead(id: string): Promise<Lead> {
  const r = await getPool().query<Lead>(
    "select id, company_name, city, contact_email, contact_name, status, contact_phone from leads where id=$1",
    [id],
  );
...
async function observation(leadId: string, company: string): Promise<string> {
  ...
  const a = await getPool().query<{ summary: string | null }>(
    "select summary from audits where lead_id=$1 order by created_at desc limit 1",
    [leadId],
  );
...
async function demoUrl(leadId: string): Promise<string | null> {
  const r = await getPool().query<{ deploy_url: string }>(
    "select deploy_url from builds where lead_id=$1 and kind='demo' and deploy_url like 'https://%' order by created_at desc limit 1",
    [leadId],
  );
...
export async function sales(leadId: string): Promise<void> {
  ...
  if (lead.status === "outreach_ready") {
    ...
    const existing = await pool.query<{ status: string }>(
      "select status from emails where lead_id=$1 and direction='outbound' and kind='outreach' order by created_at desc limit 1",
      [leadId],
    );
    if (existing.rowCount && ["awaiting_approval", "approved", "sent"].includes(existing.rows[0].status)) {
      ...
      await emitEvent({
        ...
        level: "debug",
        type: "sales.waiting",
        message: "touch 1 already exists; restored stage after rebuild",
      });
      return;
    }
    ...
    if ("blocked" in built) {
      ...
      if (already.rows[0].n === "0") {
        ...
        const phoneFirst =
          built.blocked === "no contact email on file" && lead.contact_phone && (await demoUrl(leadId));
    ...
    if (problems.length) {
      // deterministic template should never trip this; guard anyway
      await emitEvent({
        agent: "sales",
        leadId,
        level: "warn",
    ...
    const seq = await pool.query<{ id: string }>(
      "insert into email_sequences (lead_id, current_step, state) values ($1,1,'active') on conflict do nothing returning id",
      [leadId],
    );
    const seqId =
      seq.rows[0]?.id ??
      (
        await pool.query<{ id: string }>(
          "select id from email_sequences where lead_id=$1 order by created_at desc limit 1",
          [leadId],
        )
      ).rows[0]?.id;
    ...
    await emitEvent({
      ...
      leadId,
      type: "email.drafted",
      message: "touch 1 drafted (voice clean)",
    });

    if (mode() === "auto") {
      await approveAndSend(idem);
      return;
    }
  ...
  if (lead.status === "delivery_approval" && mode() === "auto") {
    await deliverFinal(leadId);
    return;
  }
  ...
  await emitEvent({
    ...
    level: "debug",
    type: "sales.waiting",
    message: `no auto action for ${lead.status}`,
  });
}
...
export async function approveAndSend(idemOrEmailId: string): Promise<{ ok: boolean; reason?: string }> {
  ...
  const em = await pool.query<{
    ...
    "select id, lead_id, subject, body_text, status, idempotency_key from emails where idempotency_key=$1 or id::text=$1 limit 1",
    [idemOrEmailId],
  );
  ...
  if (!gate.ok) {
    ...
    await emitEvent({
      ...
      level: "warn",
      type: "email.gated",
      message: gate.reason ?? "gate failed",
    });
  ...
  await pool.query("update emails set status='sent', sent_at=now(), provider_message_id=$2 where id=$1", [
    e.id,
    sent.providerMessageId,
  ]);
  // schedule Touch 2 (the operator call) for the next business touch
  await pool.query(
    "update email_sequences set current_step=1, next_send_at=now()+interval '1 day' where lead_id=$1",
    [e.lead_id],
  );
  await emitEvent({
    agent: "sales",
    leadId: e.lead_id,
    type: "email.sent",
    message: `touch 1 sent${MOCK() ? " (mock /tmp/outbox)" : ""}`,
  });
  if (lead.status === "awaiting_approval" || lead.status === "outreach_ready")
    await advanceLead(e.lead_id, "contacted", { agent: "sales" });
  await notifyOperator({
    type: "call_due",
    title: `Call ${lead.company_name} (Touch 2) — sheet on the lead page`,
    leadId: e.lead_id,
  });
  return { ok: true };
}
...
export async function rejectDraft(emailId: string): Promise<void> {
  await getPool().query("update emails set status='failed' where id=$1 and status='awaiting_approval'", [
    emailId,
  ]);
}
...
export async function ingestReply(leadId: string, text: string, forcedClass?: string): Promise<void> {
  const pool = getPool();
  const lead = await getLead(leadId);
  const outreach = await pool.query<{ id: string }>(
    "select id from emails where lead_id=$1 and direction='outbound' order by created_at desc limit 1",
    [leadId],
  );
  ...
  await pool.query(
    "insert into replies (email_id, classification, classified_by, raw) values ($1,$2,$3,$4)",
  ...
  if (classification === "unsubscribe" || classification === "not_interested") {
    ...
    await emitEvent({
      ...
      message: `${classification} -> suppressed + halted`,
    });
    if (lead.status !== "closed_lost" && lead.status !== "nurture") {
      await advanceLead(leadId, classification === "unsubscribe" ? "nurture" : "nurture", {
        agent: "sales",
      }).catch(() => undefined);
    }
    return;
  ...
  await emitEvent({
    ...
    leadId,
    type: "reply.classified",
    message: `${classification} -> operator notified`,
  });
  ...
  if (
    ...
    (lead.status === "contacted" || lead.status === "awaiting_approval")
  ) {
...
async function classifyReply(text: string, leadId: string): Promise<string> {
  const raw = await llm({
    ...
    maxTokens: 20,
    system:
      "Classify the email reply into exactly one token: interested, question, not_interested, unsubscribe, auto_reply, other. Output only the token.",
    prompt: text,
...
export async function ingestBooking(
  leadId: string,
  opts: { title?: string; startTime?: string; attendee?: any } = {},
): Promise<void> {
  const pool = getPool();
  const lead = await getLead(leadId);
  await pool.query(
...
export async function deliverFinal(leadId: string): Promise<void> {
  ...
  const footer = canSpamFooter() ?? "";
  const body = [
    `Hey ${firstName(lead.company_name) ?? "there"},`,
    "",
    `Your new site is live and pointed at your domain: ${link ?? "[link]"}`,
    "",
    `I'll keep it fast and updated. Reply here any time.`,
    "",
    loadAgencyFacts().identity.operator_first_name,
    footer,
  ].join("\n");
  ...
  if (lead.contact_email) {
    ...
    if (gate.ok) {
      ...
      await pool.query(
        "update emails set status='sent', sent_at=now(), provider_message_id=$2 where idempotency_key=$1",
        [idem, sent.providerMessageId],
      );
    }
  }
