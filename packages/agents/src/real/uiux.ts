...
import { MOCK, designGuidance, llm } from "@autopilot/adapters";
import { BLOCK_BY_ID, type Preset, blockDataSatisfied, presetForIndustry } from "@autopilot/blocks";
...
async function assignLook(
  leadId: string,
  preset: Preset,
  city: string | null,
  preferredHue: number | null = null,
): Promise<LookRow> {
  const pool = getPool();
  ...
  const looks = (
    await pool.query<LookRow>(
      "select id, name, palette, type_pairing, hero_variant from looks where preset = $1 order by name",
      [preset.id],
    )
  ).rows;
...
export async function uiux(leadId: string): Promise<void> {
  ...
  const solution = (
    await pool.query("select * from solutions where lead_id = $1 order by created_at desc limit 1", [leadId])
  ).rows[0];
  const audit = (
    await pool.query(
  ...
  if (!preset.live) {
    await notifyOperator({
      ...
      title: `${lead.company_name}: ${preset.label} template not built yet`,
      leadId,
    });
  }
  ...
  const reviewSignal =
    solution?.differentiators?.[0] ??
    (lead.review_count ? `${lead.review_count} Google reviews at ${lead.rating ?? "?"}` : null);
  const facts = {
  ...
  const raw = await llm({
    ...
    mockResponse: JSON.stringify({
      ...
      hero: {
        headline: `${facts.primary_service} in ${lead.city ?? "your area"}`,
        subhead: reviewSignal ? `${reviewSignal}. One tap to call.` : "Fast, honest work. One tap to call.",
      },
  ...
  const factBag: Record<string, unknown> = {
    ...
    city: lead.city,
    primaryService: facts.primary_service,
    reviewCount: lead.review_count,
    rating: lead.rating,
    services: true,
    reviews: undefined,
    photos: undefined,
    mapQuery: lead.company_name,
  };
  ...
  const blocks = preset.blockSequence
    ...
    .map((id) => {
      ...
      const dataKnownAtDesign =
        def.structural || ["reviews", "gallery"].includes(id) || blockDataSatisfied(def, factBag);
      if (!dataKnownAtDesign) return null;
      const block: { block: string; source: string; copy?: unknown } = {
        block: id,
        source: def.needs.length ? def.needs.join(",") : "structural",
      };
  ...
  await pool.query(
    ...
    [
      ...
      JSON.stringify([{ page: "Home", blocks }]),
      look.id,
    ],
  );
  await emitEvent({
    ...
    leadId,
    type: "design.ready",
    message: `look '${look.name}' + ${blocks.length} blocks${skillGrounded ? " (skill-grounded)" : ""}${brandColor ? ` (matched to their brand ${brandColor.hex})` : ""}${MOCK() ? " (mock)" : ""}`,
    payload: { look: look.name, preset: preset.id, skill_grounded: skillGrounded },
  });
