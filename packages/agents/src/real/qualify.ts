...
async function probeSiteWeak(url: string): Promise<{ weak: boolean; why: string }> {
  try {
    ...
    if (html.length < 300 || /window\.location\.href=.{0,4}\/lander/i.test(html)) return { weak: true, why: "parked/empty page" };
...
export async function qualify(leadId: string): Promise<void> {
  ...
  await pool.query("update leads set score = $2, score_breakdown = $3 where id = $1", [leadId, score, JSON.stringify(breakdown)]);

  // no personalization material at all -> disqualify (contract §4)
  if ((lead.photos ?? []).length === 0 && reviews === 0) {
    await pool.query("update leads set disqualify_reason = 'nothing_real_to_personalize' where id = $1", [leadId]);
    await advanceLead(leadId, "disqualified", { agent: "qualify", reason: "nothing_real_to_personalize" });
    return;
  }

  await emitEvent({ agent: "qualify", leadId, type: "lead.scored", message: `score ${score} (site: ${gapWhy})`, payload: breakdown });
  if (score >= icp.qualify_threshold) {
    await advanceLead(leadId, "qualified", { agent: "qualify" });
  } else {
    await pool.query("update leads set disqualify_reason = $2 where id = $1", [leadId, `below_threshold_${score}`]);
    await advanceLead(leadId, "disqualified", { agent: "qualify", reason: `score ${score} < ${icp.qualify_threshold}` });
  }
}
