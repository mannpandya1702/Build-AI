...
import { type LeadStatus, advanceLead, emitEvent, getPool, notifyOperator } from "@autopilot/core";
...
async function scrape(leadId: string): Promise<void> {
  ...
  await pool.query(
    ...
    [
      leadId,
      JSON.stringify([{ author: "Fixture Reviewer", rating: 5, text: "Great work, fast and honest." }]),
    ],
  );
...
async function uiux(leadId: string): Promise<void> {
  await sleep(300);
  await getPool().query("insert into designs (lead_id, brand, sitemap, page_specs) values ($1,$2,$3,$4)", [
    leadId,
    JSON.stringify({
      palette: { brand: "180 56 13" },
      fonts: { display: "Bricolage Grotesque", body: "Source Sans 3" },
      tone: "direct, local",
    }),
    JSON.stringify(["Home", "Services", "About", "Contact"]),
    JSON.stringify([
...
async function qa(leadId: string): Promise<void> {
  ...
  if (b.rowCount) {
    await getPool().query("insert into qa_reports (build_id, passed, checks) values ($1, true, $2)", [
      b.rows[0].id,
      JSON.stringify({ links: true, console_errors: 0, responsive: true, lighthouse_perf: 92 }),
    ]);
  }
  ...
  if (kind === "demo") {
    ...
    await notifyOperator({
      ...
      title: "Final build ready for delivery approval",
      leadId,
    });
  }
}
  ...
  switch (lead.status) {
    case "outreach_ready": {
      ...
      await emitEvent({
        ...
        leadId,
        type: "email.drafted",
        message: "touch 1 queued for approval (mock)",
      });
    ...
    case "awaiting_approval": {
      ...
      await emitEvent({
        ...
        leadId,
        type: "email.sent",
        message: "touch 1 sent (mock auto-approve)",
      });
    ...
    case "delivery_approval": {
      ...
      await emitEvent({
        ...
        leadId,
        type: "delivery.sent",
        message: "delivered (mock auto-approve)",
      });
    ...
    default:
      await emitEvent({
        ...
        level: "warn",
        type: "sales.noop",
        message: `no action for ${lead.status}`,
      });
  }
}
